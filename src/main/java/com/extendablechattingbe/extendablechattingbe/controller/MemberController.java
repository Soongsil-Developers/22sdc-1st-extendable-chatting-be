...
public class MemberController {
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "201", description = "회원을 성공적으로 등록", content = @Content(schema = @Schema(implementation = MemberResponse.class))),
    })
    @PostMapping("/members")
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "특정 메세지를 성공적으로 획득", content = @Content(schema = @Schema(implementation = RoomMemberResponse.class))),
        @ApiResponse(responseCode = "404", description = "입력한 정보가 데이터 베이스에 저장되어있지 않습니다.", content = @Content(schema = @Schema(implementation = SimpleResponseDTO.class)))
    })
...
//    }
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "특정 멤버가 속한 방 목록 획득 성공", content = @Content(schema = @Schema(implementation = RoomResponse.class))),
        @ApiResponse(responseCode = "404", description = "입력한 정보가 데이터 베이스에 저장되어있지 않습니다.", content = @Content(schema = @Schema(implementation = SimpleResponseDTO.class)))
    })
    //멤버가 속한 방 찾기
